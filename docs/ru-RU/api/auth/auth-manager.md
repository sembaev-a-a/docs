...
- **Аутентификатор (`Authenticator`)**: сущность, хранящаяся в базе данных, связанная с конфигурацией определённого типа аутентификации. Для одного типа может существовать несколько аутентификаторов, каждый из которых предоставляет отдельную схему аутентификации.
...
##### Настройки Менеджера аутентифиакции (AuthManagerOptions)
...
Промежуточное ПО аутентификации. Получает текущий аутентификатор и выполняет аутентификацию пользователя.
